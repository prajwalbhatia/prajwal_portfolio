...
import { ExperienceAccordion } from '@/components/preview/experience-accordion'
import { IdBadge } from '@/components/preview/id-badge'
import { StackIcon, stackHue } from '@/components/preview/stack-icons'
import { TerminalCard } from '@/components/preview/terminal-card'
...
export const metadata: Metadata = {
  title: 'Preview',
  // Throwaway prototype. Must never be indexed or linked.
...
/**
 * Local for now. `profile.tagline` still reads "I make slow React apps fast."
 * and is rendered by the live dark hero AND the OG card, so changing it there
 * would alter both before this design is agreed. Promoting this design means
 * moving this string into content/profile.ts.
 */
const PREVIEW_TAGLINE =
  'Turning complexity into simple, scalable, and performant experiences.'

/**
 * Prototype-local overrides so `content/projects.ts` stays untouched until
 * the design is approved. Outcome-first headlines, as hers are.
 *
    ...
    <section id="explainers" aria-labelledby="explainers-heading" className="rule-t">
      <div className="shell gutter py-14">
        ...
        {items.length > 0 ? (
          ...
          /* No YOUTUBE_API_KEY in this environment. Empty 9:16 frames so the
             layout is still reviewable — the real page hides the whole
             section instead of showing placeholders. */
          <div>
            <ul className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {[0, 1, 2, 3].map((i) => (
                <li
                  key={i}
                  className="flex aspect-9/16 items-center justify-center border border-rule bg-raise"
                >
                  <span className="label text-[0.62rem] text-dim">9:16</span>
                </li>
              ))}
            </ul>
            <p className="label mt-4 text-[0.66rem] text-dim">
              Placeholders — no YOUTUBE_API_KEY set. Live, this section hides itself.
            </p>
          </div>
        )}
...
export default async function PreviewPage() {
  ...
  return (
    <div data-preview className="min-h-screen bg-ground text-ink">
      <p className="label border-b border-rule bg-raise px-4 py-2 text-center text-[0.62rem] text-muted">
        Prototype · light palette scoped to this route · not indexed
      </p>
