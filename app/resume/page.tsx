...
export default function ResumePage() {
  return (
    <article className="shell gutter py-12 max-w-[62rem]">
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-5 border-b border-line-2">
        <div>
      ...
      <section className="py-6 border-b border-line">
        ...
        <p className="text-sm leading-relaxed text-dim max-w-[75ch]">
            ...
            <div key={role.id} className="grid gap-2 sm:grid-cols-[1fr_11rem]">
              <div>
                <h3 className="font-semibold text-text">
                  {role.title},{' '}
                  <span className="font-normal italic text-dim">{role.company}</span>
                </h3>
      ...
      <section className="py-6 border-b border-line">
        ...
        <div className="grid gap-1 sm:grid-cols-[1fr_11rem]">
          <div>
            ...
            <p className="text-sm italic text-dim">{education.institution}</p>
          </div>
          <div className="label text-muted sm:text-right tabular-nums leading-relaxed">
      ...
      <section className="py-6 border-b border-line">
        ...
        <dl className="grid gap-x-8 gap-y-3 sm:grid-cols-2">
          {skillGroups.map((g) => (
            <div key={g.group}>
              <dt className="font-semibold text-sm text-text mb-0.5">{g.group}</dt>
              <dd className="text-sm text-dim leading-relaxed">{g.items}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <section className="py-6">
        ...
        <ul className="flex flex-col gap-1.5">
          {awards.map((a) => (
            <li key={a.title} className="text-sm text-dim">
              <span className="font-semibold text-text">{a.title}</span> — {a.detail} ({a.year})
            </li>
          ))}
        </ul>
      </section>
